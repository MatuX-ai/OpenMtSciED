...
### ✅ 测试场景 3：统一资源库 (Resource Explorer)

**访问路径**: `http://localhost:4200/resource-explorer`

**旧路由兼容**（保留书签）:
- `/material-library?search=foo` → 重定向并保留 search
- `/tutorial-library` → 重定向并附加 `type=tutorial`
- `/resource-browser` → 重定向
...
1. **首次使用应用**
   ```
   启动应用 → 初始化向导 → 填写信息 → 完成设置 → 进入教程库
   

2. **创建第一个课程**
   ```
   教程库页面 → 点击新建 → 填写课程信息 → 保存 → 验证课程出现在列表中
