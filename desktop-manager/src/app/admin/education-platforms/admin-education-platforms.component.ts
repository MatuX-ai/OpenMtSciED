  ...
  async loadPlatformStatus(): Promise<void> {
    ...
    try {
      const status = await firstValueFrom(
        this.http.get<Record<string, PlatformStatus>>('/api/v1/education-platforms/status')
      );
      
      const platformsArray = Object.values(status);
      this.platforms.set(platformsArray);
      
      // 检查定时任务状态
      const scheduleStatus = await firstValueFrom(
        this.http.get<{ scheduled_tasks_active: boolean }>('/api/v1/education-platforms/schedule/status')
      );
      this.scheduleActive.set(scheduleStatus.scheduled_tasks_active);
  ...
  async toggleSchedule(): Promise<void> {
    try {
      if (this.scheduleActive()) {
        await firstValueFrom(
          this.http.post('/api/v1/education-platforms/schedule/stop', {})
        );
        this.scheduleActive.set(false);
        this.snackBar.open('定时任务已停止', '关闭', { duration: 2000 });
      } else {
        await firstValueFrom(
          this.http.post('/api/v1/education-platforms/schedule/start', {})
        );
        this.scheduleActive.set(true);
        this.snackBar.open('定时任务已启动', '关闭', { duration: 2000 });
      }
    } catch (error) {
      console.error('切换定时任务状态失败:', error);
      this.snackBar.open('切换定时任务状态失败', '关闭', { duration: 3000 });
    }
  }
