...
import { WebusbFlashComponent } from '../webusb-flash/webusb-flash.component';

@Component({
  selector: 'app-search-map',
  standalone: true,
  imports: [CommonModule, FormsModule, AiTutorComponent, WebusbFlashComponent],
  template: `
    <div class="container">
      <div class="controls">
        <input
      ...
      <app-ai-tutor [contextNode]="selectedNode"></app-ai-tutor>
      <app-webusb-flash #webusbFlash></app-webusb-flash>
    </div>
  `,
  styles: [
    `
...
export class SearchMapComponent implements OnInit, OnDestroy {
  @ViewChild('webusbFlash') webusbFlash!: WebusbFlashComponent;
  ...
  loadGraphData() {
    this.http.get('/api/marketing/graph-data').subscribe({
      next: (data: any) => {
        this.graphData = data;
        const option = {
          tooltip: {},
          legend: { data: data.categories.map((a: any) => a.name) },
          series: [
            {
              type: 'graph',
              layout: 'force',
  ...
  download(id: string) {
    window.open(`/api/marketing/download/${id}`, '_blank');
  }
...
void loop() {
  ...
  delay(2000);
    ...
    this.webusbFlash.show(mockProject);
  }
