...
export async function POST() {
  ...
  try {
    ...
    for (const bank of STEM_QUESTION_BANKS) {
      ...
      for (let idx = 0; idx < bank.questions.length; idx++) {
        const q = bank.questions[idx];
        const questionId = `stem-q-${bank.category}-${String(idx + 1).padStart(3, '0')}`;
        ...
        if (result.records.length > 0) {
          console.log(`  ✅ [${idx + 1}/${bank.questions.length}] ${q.content.substring(0, 40)}...`);
          totalImported++;
        } else {
          console.log(`  ❌ [${idx + 1}/${bank.questions.length}] 导入失败`);
        }
      }
    }
