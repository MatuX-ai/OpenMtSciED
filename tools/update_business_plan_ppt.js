...
// 加载现有PPT
pptx.load('g:/iMato/scripts/MatuX_创业大赛商业计划书_完整版.pptx').then(() => {
  console.log('成功加载现有PPT');
  ...
  console.log(`当前PPT共有 ${slideCount} 页`);
  ...
  // 保存更新后的PPT
  const outputPath = 'g:/iMato/scripts/MatuX_创业大赛商业计划书_完整版_更新版.pptx';
  pptx.writeFile({ fileName: outputPath })
    .then(() => {
      console.log('PPT更新成功！');
      console.log(`新PPT共 ${pptx.slides.length} 页`);
      console.log(`保存路径: ${outputPath}`);
      console.log(`新增幻灯片: ${pptx.slides.length - slideCount} 页`);
    })
    .catch(err => {
      console.error('PPT更新失败:', err);
    });
}).catch(err => {
  console.error('加载PPT失败:', err);
  console.log('将创建新的PPT文件...');
  
  // 如果加载失败，创建新文件
  console.log('请检查原PPT文件是否存在，或使用新生成的文件');
});
